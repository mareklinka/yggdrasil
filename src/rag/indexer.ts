...
export class Indexer {
	...
	public constructor(config: IndexerConfig, vault: VaultAdapter) {
		this.#config = config;
		this.#vault = vault;
		this.#embedder = new Embedder({
			endpoint: config.embedEndpoint,
	...
	public async reindex(onProgress: ProgressCallback): Promise<IndexResult> {
		...
		try {
			...
			// Phase 2: Initialize vector store
 ...
 * @param vault - The vault adapter for file operations.
 * @returns A new Indexer instance.
 */
export function createIndexer(config: IndexerConfig, vault: VaultAdapter): Indexer {
	return new Indexer(config, vault);
}
