...
export const { init: initEmbedder, get: getEmbedder } = (function (): {
  init: (this: void, config: EmbedderConfig) => Embedder;
  get: (this: void) => Embedder;
} {
  let instance: Embedder | null = null;

  return {
    init: function (this: void, config: EmbedderConfig): Embedder {
      return (instance ??= new Embedder(config));
    },
    get: function (this: void): Embedder {
      if (!instance) {
  ...
  async #requestEmbeddings(texts: Array<string>): Promise<EmbeddingResponse> {
    ...
    try {
      ...
      if (response.status >= 400) {
        ...
        throw new Error(
          `Embedding API error (${response.status}): ${errorMessage}`,
        );
      }
