...
import { Indexer, type IndexerConfig, type IndexProgress, type IndexResult } from '../src/rag/indexer';
import type { EmbeddingResult } from '../src/rag/embedder';
...
describe('Indexer', () => {
	...
	const mockVault = {
		getMarkdownFiles: vi.fn<Array<never>, Array<{ path: string }>>().mockReturnValue([]),
		read: vi.fn<{ file: { path: string } }, Promise<string>>().mockResolvedValue(''),
	};
	...
	beforeEach(async () => {
		...
		indexer = new Indexer(mockConfig, mockVault, mockEmbedder as any, mockFileSystem);
	...
	describe('reindex', () => {
		...
		it('should handle file read errors gracefully', async () => {
			...
			mockEmbedder.embedMany.mockResolvedValue({
				results: [{ embedding: Array(1024).fill(0.01), text: 'Readable content' }],
				errors: [],
			});
		...
		it('should report progress updates during indexing', async () => {
			...
			const indexingUpdates = progressUpdates.filter((p) => p.phase === 'indexing');
		...
		it('should store chunks with correct metadata', async () => {
			mockVault.getMarkdownFiles.mockReturnValue([{ path: 'campaigns/npc.md' }]);
			mockVault.read.mockResolvedValue('Grommet is a dwarf warrior.');

			const mockEmbedding = Array(1024).fill(0.01);
			mockEmbedder.embedMany.mockResolvedValue({
				results: [{ embedding: mockEmbedding, text: 'Grommet is a dwarf warrior.' }],
				errors: [],
			});
			...
			expect(mocks.mockVectorStore.addChunks).toHaveBeenCalled();
			const storedChunks: Array<StoredChunk> = mocks.mockVectorStore.addChunks.mock.calls[0][0];
		...
		it('should handle indexing errors and report failure', async () => {
			...
			mocks.mockVectorStore.saveToDisk.mockRejectedValueOnce(new Error('Disk write failed'));
	...
	describe('constructor', () => {
		it('should create indexer with custom embedder config', () => {
			const customConfig: IndexerConfig = {
				dbPath: '/custom/path.json',
				dimensions: 512,
				embedEndpoint: 'http://custom:8080',
